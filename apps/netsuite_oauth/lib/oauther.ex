defmodule NetsuiteOAuth do
  defmodule Credentials do
    ...
    @type t :: %__MODULE__{
            ...
            realm: nil | String.t(),
            method: :hmac_sha256 | :rsa_sha1 | :plaintext
          }
  end
  ...
  def header(params) do
    ...
    oauth_params = oauth_params |> List.insert_at(0, {"realm", realm()})
  ...
  def signature(verb, url, params, %Credentials{method: :hmac_sha256} = creds) do
    ...
    |> compose_key()
    |> IO.inspect(label: "composed key")
    |> hmac_sha(base_string(verb, url, params))
    |> IO.inspect(label: "HMAC_SHA_RESULT")
    |> Base.encode64()
    |> IO.inspect(label: "BASE64 HMAC_SHA256")
  end
  ...
  if Code.ensure_loaded?(:crypto) and function_exported?(:crypto, :mac, 4) do
    defp hmac_sha(key, data) do
      IO.inspect([key, data], label: "HMAC_SHA_INPUTS")
  ...
  defp base_string(verb, url, params) do
    ...
    |> IO.inspect(label: "Base String for HMAC Data")
    |> Stream.map(&normalize/1)
    |> Enum.map_join("&", &percent_encode/1)
    |> IO.inspect(label: "Normalize and encode base string")
  end
