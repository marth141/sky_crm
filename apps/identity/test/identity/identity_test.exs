defmodule IdentityTest do
  ...
  describe "register_user/1" do
    ...
    test "registers users with a hashed password" do
      ...
      {:ok, user} = Identity.register_user(%{email: email, password: valid_user_password()})
    ...
    test "sends token through notification", %{user: user} do
      token =
        extract_user_token(fn url ->
          Identity.deliver_update_email_instructions(user, "current@example.com", url)
        end)
  ...
  describe "update_user_email/2" do
    setup do
      ...
      token =
        extract_user_token(fn url ->
          Identity.deliver_update_email_instructions(%{user | email: email}, user.email, url)
        end)
  ...
  describe "update_user_password/3" do
    ...
    test "deletes all tokens for the given user", %{user: user} do
      _ = Identity.generate_session_token(user)
  ...
  describe "users" do
    ...
    @valid_attrs %{confirmed_at: ~N[2010-04-17 14:00:00], email: "some email", is_admin: true}
    @update_attrs %{
