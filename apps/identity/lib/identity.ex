defmodule Identity do
  @moduledoc """
  Identity is responsible for verifying who a person is and the basics of what they can do (is_admin authorization).
  """
  import Ecto.Query, warn: false
  alias Identity.{User, UserToken, UserNotifier}
  ...
  @doc """
  Returns a boolean indicating whether or not any active administrators are present in the system.

  Used to tell us whether or not we should allow a public registration of a user.
  """
  def any_active_admins? do
    Repo.all(
      from user in User,
  ...
  def deactivate_user(user_id) do
    ...
    Ecto.Multi.new()
    |> Ecto.Multi.update(:user, User.changeset(user, %{is_active: false}))
  ...
  def list_users() do
    base_user_query()
    |> order_by([user: u], desc: u.is_active, asc: u.email)
    |> Repo.all()
  end

  defp base_user_query do
  ...
  def register_user_admin(attrs) do
    %User{}
    |> User.registration_changeset(attrs)
    |> User.changeset(%{is_admin: true})
    |> Repo.insert()
