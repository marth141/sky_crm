defmodule Identity.User do
  ...
  def changeset(user, attrs) do
    user
    |> cast(attrs, [:is_active, :is_admin, :email])
    |> validate_required([:is_active, :is_admin, :email])
  end
  ...
  defp maybe_generate_password(changeset) do
    unless get_change(changeset, :password) do
      changeset
      |> put_change(:password, Argon2.gen_salt() |> Base.url_encode64())
    else
      changeset
    end
  end
  ...
  """
  def valid_password?(%Identity.User{hashed_password: hashed_password}, password)
