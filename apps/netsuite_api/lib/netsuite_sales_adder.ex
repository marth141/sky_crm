defmodule NetsuiteApi.SalesAdder do
  use Ecto.Schema
  import Ecto.Changeset

  @fields ~w(
    netsuite_sales_adder_id
    isInactive
    name
    scriptId
  )a

  schema "netsuite_sales_adders" do
    field(:netsuite_sales_adder_id, :string)
  ...
  def create(%Ecto.Changeset{valid?: true} = attrs) do
    attrs
    |> Repo.insert!(on_conflict: :replace_all, conflict_target: :netsuite_sales_adder_id)
  end
  ...
  defp new(changeset_changes) do
    %__MODULE__{
      netsuite_sales_adder_id: changeset_changes[:netsuite_sales_adder_id],
