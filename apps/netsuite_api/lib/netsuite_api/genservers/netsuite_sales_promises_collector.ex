defmodule NetsuiteApi.SalesPromisesCollector do
  @moduledoc """
  ...
  def refresh_cache() do
    Repo.delete_all(NetsuiteApi.SalesPromise)

    stream_list_from_netsuite("customlist_skl_promises")
    |> Task.async_stream(&relabel_id/1)
    |> Task.async_stream(&insert_or_update_postgres/1, timeout: :infinity)
    |> Stream.run()

    IO.puts("\n \n ======= Netsuite Sales Promises Refreshed ======= \n \n")
    schedule_poll()
    :ok
  end
  ...
  defp insert_or_update_postgres({:ok, netsuite_record}) do
    try do
      existing =
        Repo.get_by!(NetsuiteApi.SalesPromise,
          netsuite_sales_adder_id: netsuite_record["netsuite_sales_promise_id"]
        )

      existing
      |> NetsuiteApi.SalesPromise.update(netsuite_record)
    rescue
      _ -> NetsuiteApi.SalesPromise.create(netsuite_record)
    end
  end
  ...
  defp relabel_id(netsuite_record) do
    netsuite_record
    |> Enum.map(fn {k, v} -> if(k == "id", do: {"netsuite_sales_promise_id", v}, else: {k, v}) end)
    |> Map.new()
    |> Map.delete("links")
  end
