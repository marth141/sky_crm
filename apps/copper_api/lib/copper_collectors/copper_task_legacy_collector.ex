defmodule CopperApi.LegacyTasks do
  @moduledoc """
  ...
  def list_tasks_ets() do
    :ets.select(:copper_tasks_legacy, match_all_without_view_state())
  end
  ...
  def fetch_ets(task_id) do
    [{_key, task, _assigned}] = :ets.lookup(:copper_tasks_legacy, task_id)
    task
  end
  ...
  def get_tasks_for_user_id(user_id) do
    :ets.select(:copper_tasks_legacy, match_on_tasks_for_user_id(user_id))
  end
  ...
  def init(_opts) do
    tid = :ets.new(:copper_tasks_legacy, [:named_table, :set, :public, read_concurrency: true])
    :ets.insert(:copper_tasks_legacy, {:view_state, default_view_state()})
  ...
  defp insert_view_state_to_ets(key, value) when is_atom(key) do
    ...
    :ets.insert(:copper_tasks_legacy, {:view_state, view_state})
  end
  ...
  def fetch_view_state() do
    [{_key, view_state}] = :ets.lookup(:copper_tasks_legacy, :view_state)
    view_state
  end

  def insert_to_ets(%CTask{id: id, assignee_id: assignee_id} = task) do
    :ets.insert(:copper_tasks_legacy, {id, task, assignee_id})
  end
  ...
  def refresh_legacy() do
    ...
    set_live_status()
    Messaging.publish("CopperApi", :copper_tasks_legacy_updated)
