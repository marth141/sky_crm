    ...
    <div id="navatar-<%= @id %>"
      ...
      <div>
        <button @click="open = !open" class="max-w-xs flex items-center text-sm rounded-full text-white focus:outline-none focus:shadow-solid">
          <div class="flex items-center rounded-full bg-rose-200 h-8 w-8">
            <svg fill="currentColor" viewBox="0 0 20 20" class="text-gray-500 m-1">
      ...
      <div x-show="open"
        ...
        <div class="origin-top-right absolute right-0 mt-2 mr-2 w-56 bg-white rounded-lg shadow-md py-2 text-gray-800 text-sm">
          <ul>
            ...
            <%= live_redirect to: Routes.user_profile_path(@socket, :show) do %>
              <li class="flex group block px-4 py-2 hover:bg-rose-100">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 text-gray-600 group-hover:text-rose-700 fill-current" viewBox="0 0 24 24">
                ...
                <p class="px-4 group-hover:text-rose-700">Profile</p>
              </li>
            <% end %>

            <%= live_redirect to: Routes.user_account_settings_path(@socket, :show) do %>
              <li class="flex group block px-4 py-2 hover:bg-rose-100">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 text-gray-600 group-hover:text-rose-700 fill-current" viewBox="0 0 24 24">
                ...
                <p class="px-4 group-hover:text-rose-700">Account Settings</p>
              </li>
            <% end %>

            <%= link to: Routes.user_session_path(@socket, :delete), method: :delete do %>
              <li class="flex group block px-4 py-2 hover:bg-rose-100">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 text-gray-600 group-hover:text-rose-700 fill-current" viewBox="0 0 24 24"><path d="M16 13L16 11 7 11 7 8 2 12 7 16 7 13z"/>
                ...
                <p class="px-4 group-hover:text-rose-700">Sign Out</p>
              </li>
            <% end %>
